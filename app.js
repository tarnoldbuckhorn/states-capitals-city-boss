...
function updateBossPanel(message = null) {
  ...
  els.bossActionBtn.textContent = "Quiz Battle";
  if (message) {
...
function handleBossAction() {
  ...
  setActivePanel("quiz");
  updateBossPanel("Answer quiz questions to damage the boss!");
  els.result.textContent = `Answer correctly to strike ${state.boss.name}.`;
  saveState();
}
...
async function init() {
  ...
  els.bossActionBtn.addEventListener("click", handleBossAction);
